...
func registerReservationRoutes(r *gin.Engine, authService *auth.Service, pool *pgxpool.Pool) {
	...
	read.Use(requireSession(authService), enforceForcePasswordChange(), requireRoles(authService, allRead...))
	{
		...
		read.GET("/exceptions/history", h.listExceptionHistory)
		read.GET("/exceptions", h.listOpenExceptions)
		read.GET("/exceptions/:id", h.getException)
	}
	...
	dispatchWrite.Use(requireSession(authService), enforceForcePasswordChange(), requireRoles(authService, auth.RoleDispatch))
	{
		dispatchWrite.POST("/exceptions/:id/acknowledge", h.acknowledgeException)
	}
}
